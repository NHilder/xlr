#' Convert beta types to a openxlsx workbook.
#'
...
beta_to_workbook <- function(x,
  ...
  if (is_bare_list(x)){
    ...
    lapply(x, function(y){
      type_abort(y,
                 \(x) is.data.frame(x) || is_tibble(x) || is_beta_table(x),
                 NULL,
                 string_type = "<data.frame>, <tibble>, or <beta_table>",
                 call = call)
  ...
  lapply(names(x),
         function(sheet_name, call = caller_env()) {
           data <- x[[sheet_name]]
           # must be a beta_table or data.frame
           if (is_beta_table(data)) {
             beta_table_to_sheet(data,
                                 wb,
                                 sheet_name,
                                 TOC = TOC,
...
add_TOC_to_workbook <- function(wb,
                                ...
                                TOC_title = NA,
                                TOC_sheet_name = "Table of Contents",
                                TOC_title_format = beta_format(font_size = 14, text_style = "bold"),
                                call = caller_env()){
  ...
  addStyle(wb,
           TOC_sheet_name,
           style = beta_format_to_openxlsx_format(TOC_title_format,
                                                  "TEXT"),
           rows = 1,
           cols = 1)
...
beta_table_to_sheet <- function(x,
                                wb,
                                sheet_name,
                                TOC = FALSE,
  ...
  if (ncol(data) == 0){
    cli_abort(c("i" = "In argument: {.arg x}.",
                "!" = "Your <beta_table> {.val {sheet_name}} must contain columns."),
              call = call)
  }
  if (nrow(data) == 0){
    cli_warn(c("i" = "In argument: {.arg x}.",
                "Your <beta_table> {.val {sheet_name}} has no data in it's rows."),
             call = call)
  }
  ...
  # Add the BETA data to the sheet
  ...
  if (title_option) {
    ...
    addStyle(wb,
             sheet_name,
             style = beta_format_to_openxlsx_format(pull_title_format(x),
                                                    "TEXT"),
             rows = 1,
             cols = 1)
  }
  ...
  if (footnote_option){
    ...
    addStyle(wb,
             sheet_name,
             style = beta_format_to_openxlsx_format(pull_footnote_format(x),
                                                    "TEXT"),
             cols = 1,
  ...
  # from the normal beta_formatting, as we are applying styles to the whole
  # table.
  ...
  addStyle(wb,
           sheet_name,
           beta_format_to_openxlsx_format(pull_column_heading_format(x),
                                          "TEXT"),
           rows = 2,
           cols = 1:ncol(data),
           gridExpand = TRUE,
...
get_border_styles <- function(body_styling,
  ...
  type_abort(body_styling,is_beta_format,beta_format())
...
data_to_worksheet <- function(x,
  ...
  # also convert all the BETA_types to their basic type
  df <- convert_beta_type_to_R(x,
                               call)
...
convert_beta_type_to_R <- function(x,
                                   call = caller_env()){
  x |>
    lapply(\(x){
        if (is_beta_percent(x)) return(vec_cast(x,double()))
        else if (is_beta_double(x)) return(vec_cast(x,double()))
        else if (is_beta_integer(x)) return(vec_cast(x,integer()))
        else if(is_beta_vector(x)) return(get_beta_vector_data(x))
        else return(x)
      }) |>
...
column_to_style <- function(col){
  # first we have rules for all the BETA types (and integers)

  if (is_beta_percent(col)){
    column_cell_format <- paste0(generate_dp(pull_dp(col)),"%")
  }
  else if(is_beta_integer(col) || is.integer(col)){
    column_cell_format <- "0"
  }
  else if(is_beta_double(col)){
    column_cell_format <- generate_dp(pull_dp(col))
  }
  else if(is_beta_vector(col)){
    # Lets actually pull out the underlying data and call it again
    col <- get_beta_vector_data(col)
  ...
  # now lets pull out the beta format information if it exists,
  # if it doesn't, we generate an empty beta_format
  format_data <- pull_style(col)
  if (is.null(format_data)){
    format_data <- beta_format()
  }
  # new lets create the format
  beta_format_to_openxlsx_format(format_data,
...
beta_format_to_openxlsx_format <- function(x,
                                           cell_format = "GENERAL",
                                           call = caller_env(2)){

  type_abort(x,is_beta_format,beta_format(),call = call)

  text_rotation <- attr(beta_format(),which="text_rotation")
  if (text_rotation == 0){
    text_rotation <- NULL
  }
