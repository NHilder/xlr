#' @title Specify formatting options for `beta_*` types
#' @description
...
#' ```{r}
#' fmt <- beta_format(text_style = c("bold", "italic"))
#' ```
#' If you would like to the text to be only \strong{bold} then:
#' ```{r}
#' fmt <- beta_format(text_style = "bold")
#' ```
#' ## Border styling
...
#' if you set:
#' ```{r}
#' fmt <- beta_format(border = c("left", "top"),
...
#' ```
#' @seealso
#' * [is_beta_format()] to test if an R object is a `beta_format`
#' * [beta_table()] to use beta formats
#'
#' @example inst/examples/beta_format.R
#' @export
beta_format <- function(font_size = 11,
                       font_colour = "black",
                       font = "calibri",
                       text_style = NULL,
                       border = NULL,
...
#' Test if an object is a `beta_format`
#' @param x An object to test
#'
#' @export
#'
#' @example inst/examples/beta_format_utils.R
is_beta_format <- function(x) inherits(x,"beta_format")


# now we define a number of generic methods

#' @export
print.beta_format <- function(x,...){
...
new_beta_format <- function(font_size = 11,
                            font_colour = "black",
                            font = "calibri",
                            text_style = NULL,
                            border = NULL,
...
# Validates the beta_format object, called in construction
validate_beta_format <- function(
    font_size,
    font_colour,
    font,
    text_style,
