...
-   `xlr_double()` to format doubles neatly.
...
For `xlr_double()` and `xlr_percent()` you can also set the number of
...
table$mpg <- xlr_double(table$mpg, dp = 0)
...
of `xlr_table`'s (or tibbles or data.frames) to Excel. This is useful
