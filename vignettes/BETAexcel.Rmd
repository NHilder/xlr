---
title: Getting started with BETAexcel
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting started with BETAexcel}
...
BETAexcel is designed to help with the analysis of survey data and
...
such as question labels which BETAexcel functions can utilise
automatically.
...
We will start by introducing `beta_table()` and go through all of its
...
#### Introduction to beta_table()

You can also see that it has a special type `beta_table()`. This is a S3
class defined by BETAexcel to help output the data to Excel. It contains
the table data, a table title and a footnote, as well as hidden data to
help format the table in Excel. You can learn more about `beta_table()`
...
`beta_vector()`. A `beta_vector` is a wrapper around other types (see
...
mulitple options this percentage will likely not add up to 100.
...
labelled data to a `beta_vector()`.
...
## beta_table(), BETA types and writing to Excel

A key part of the BETAexcel is to export data to Excel in an easy, and
user friendly way. The workhorse function in this scenario is
`beta_table()`, and if you like the default options is meant to be the
function that you will mostly use.

A `beta_table()` is a `tibble::tibble()` with an optional title, and
footnote, as well as a number of formatting options.

```{R}
clothes_opinions |>
  beta_table("This is a title",
             "this is a footnote with extra information")
```

When you call `beta_table()` it converts the elements of your table to
different `BETAexcel` types.

### Types

BETAexcel types are defined to help with the formatting of columns when
they are output to Excel. There are four difference types defined in
BETAexcel:

-   `beta_double()` to format doubles neatly.

-   `beta_integer()` to format integer data neatly.

-   `beta_percent()` to format numeric data as a percentage.

-   `beta_vector()` a general type to format data nicely without
    specific rules.

All of the above variables contain the argument `beta_format` which only
takes a `beta_format()` object. `beta_format()` allows you to control
...
the font size, colour, text position etc. with `beta_format()`. See
`?beta_format` for the full range of options.

For `beta_double()` and `beta_percent()` you can also set the number of
...
```{R}
table <- beta_table(mtcars, "A clever title", "A useful footnote")
             
# Lets update the format of the mpg column so that it displays using 0 decimal places
table$mpg <- beta_double(table$mpg, dp = 0)
...
table <- table |>
  dplyr::mutate(
    mpg = beta_double(mpg, dp = 0),
    # convert columns that are integers to beta_integer type
    across(vs:carb, ~ beta_integer(.x))
  )
```

### dplyr verbs

The `beta_table()` type is implemented to work with most `dplyr` verbs
to make working with the data as seamless as possible. To find out more
see `?betaexcel_and_dplyr`.

## Writing data to Excel

BETAexcel makes writing data to Excel easy using the `write_xlsx()`
function. This function takes either a `beta_table`, `tibble` or
...
When you output a `beta_table` with this function additional formatting
...
![Example of using a beta_table with write_xlsx, the table now has a
table, footnote and formatting!](img/beta_table_example.png)
...
table <- update_theme(table,
                      title_format = beta_format(font_colour = "red",
...
Like `openxlsx::write.xlsx()` you can also pass a list of What is
particularly useful about this function is that it can output a `list`
of `beta_table`'s (or tibbles or data.frames) to Excel. This is useful
if you are working survey data as you usually generate a long list of
tables in a list.

If you have a `list` of tables, set `TOC = TRUE` in order to generate a
table of contents from the table data.

# Putting it all together

Example of how you can use BETAexcel to analyse a survey is below:
