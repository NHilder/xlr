...
# beta_table_to_sheet() aborts if the sheets don't exist

    Code
      beta_table_to_sheet(beta_table(data.frame()), wb, "test")
    Condition
      Error:
      ! The sheet "test" does not exist in your workbook.
      i Add it first with `openxlsx::addWorkSheet()`.

# beta_table_to_sheet() creates the approriate error messages when x is missing columsn or rows
    ...
    Condition
      Error:
      i In argument: `x`.
      ! Your <beta_table> "test" must contain columns.

---

    Code
      beta_table_to_sheet(beta_table(data.frame(data = character())), wb, "test")
    Condition
      Warning:
      i In argument: `x`.
      Your <beta_table> "test" has no data in it's rows.
...
# beta_to_workbook() creates an error when you pass an empty list

    Code
      beta_to_workbook(list())
    Condition
      Error:
      i In argument: `x`.
      ! The list does not contain any elements!

# beta_to_workbook() checks the sheet_name type correctly

    Code
      beta_to_workbook(mtcars, sheet_name = 1)
    Condition
      Error:
...
---
    ...
    Code
      beta_to_workbook(mtcars, "test", 1)
    Condition
      Error:
      i In argument: `old_wb`.
      ! `old_wb` must be NULL or a openxlsx Workbook, not a number.

# beta_to_workbook() checks the overwrite_sheet type correctly

    Code
      beta_to_workbook(mtcars, "test", overwrite_sheet = "wee")
    Condition
      Error:
      i In argument: `overwrite_sheet`.
      ! `overwrite_sheet` must be TRUE or FALSE, not a string.

# beta_to_workbook() checks the TOC type correctly

    Code
      beta_to_workbook(mtcars, "test", TOC = "wee")
    Condition
      Error:
      i In argument: `TOC`.
      ! `TOC` must be TRUE or FALSE, not a string.

# beta_to_workbook() checks the TOC_title type correctly

    Code
      beta_to_workbook(mtcars, "test", TOC = TRUE, TOC_title = FALSE)
    Condition
      Error:
      i In argument: `TOC_title`.
      ! `TOC_title` must be a string, not `FALSE`.

# beta_to_workbook() checks the excel_data_table type correctly

    Code
      beta_to_workbook(mtcars, "test", excel_data_table = "TITLE")
    Condition
      Error:
      i In argument: `excel_data_table`.
      ! `excel_data_table` must be TRUE or FALSE, not a string.

# beta_to_workbook() errors when you pass a sheet_name, and a list of data

    Code
      beta_to_workbook(list(mtcars, mtcars), "test")
    Condition
      Warning:
      i In argument: `sheet_name`.
...
# beta_to_workbook() errors when you pass the wrong type in a list

    Code
      beta_to_workbook(list("asd", 123))
    Condition
      Error:
      i In argument: `y`.
      ! `y` must be <data.frame>, <tibble>, or <beta_table>, not a string.

# beta_to_workbook() errors when you pass the wrong type

    Code
      beta_to_workbook(123)
    Condition
      Error:
      i In argument: `x`.
      ! `x` must be <data.frame>, <tibble>, or <beta_table>, not a number.

# beta_to_workbook() errors when you pass a single object but not a sheetname

    Code
      beta_to_workbook(mtcars)
    Condition
      Error:
      i In argument: `sheet_name`.
