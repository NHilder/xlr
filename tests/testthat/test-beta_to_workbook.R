...
test_that("convert_beta_type_to_R() works correctly",{

  x <- data.frame(c1 = 1:10,
                  c2 = beta_percent(1:10/100),
                  c3 = beta_double(1:10),
                  c4 = beta_integer(1:10),
                  c5 = beta_vector(rep("a",10)),
                  c6 = rep("a",10))
  ...
  expect_equal(convert_beta_type_to_R(x),output)
...
test_that("beta_format_to_openxlsx_format() works correctly",{

  expect_error(beta_format_to_openxlsx_format("car"))
  ...
  expect_equal(beta_format_to_openxlsx_format(beta_format()),
               output_style)
})
...
test_that("column_to_style() works with a data.frame",{
  ...
  expect_equal(column_to_style(x[,"c1"]),
               beta_format_to_openxlsx_format(beta_format(),"0.00"))
...
test_that("column_to_style() handles beta_percent() correctly",{
  expect_equal(column_to_style(beta_percent(1:10/100)),
             beta_format_to_openxlsx_format(beta_format(),"0%"))
  expect_equal(column_to_style(beta_percent(1:10/100,2)),
               beta_format_to_openxlsx_format(beta_format(),"0.00%"))
})

test_that("column_to_style() handles beta_double() correctly",{
  expect_equal(column_to_style(beta_double(1:10,0)),
               beta_format_to_openxlsx_format(beta_format(),"0"))
  expect_equal(column_to_style(beta_double(1:10,2)),
               beta_format_to_openxlsx_format(beta_format(),"0.00"))
})


test_that("column_to_style() handles beta_integer() correctly",{
  expect_equal(column_to_style(beta_integer(1:10)),
               beta_format_to_openxlsx_format(beta_format(),"0"))
})


test_that("column_to_style() handles beta_vector() correctly",{
  expect_equal(column_to_style(beta_vector(rep("a",10))),
               beta_format_to_openxlsx_format(beta_format(),"GENERAL"))
})
...
test_that("column_to_style() handles Date() correctly",{
  expect_equal(column_to_style(rep(as.Date(c("2022-02-02","1999-05-03")),5)),
               beta_format_to_openxlsx_format(beta_format(), "dd/mm/yyyy"))
})
...
test_that("get_border_styles() the default border style works for a single border", {
  border_style <- beta_format(border = "left")
...
test_that("get_border_styles() works for multiple borders", {
  border_style <- beta_format(border = c("left","right"))
...
test_that("add_TOC_to_workbook() works", {
  ...
  # now create a work
  wb <- createWorkbook()
  x_list_data <- list("Test Sheet 1" = beta_table(mtcars,
                                                  "Test title 1"),
                      "Test Sheet 2" = beta_table(mtcars,
                                                  "Test title 2"),
                      "Test Sheet 3" = beta_table(mtcars,
                                                  "Test title 3"))
...
test_that("add_TOC_to_workbook() overwrites the TOC data if it exists",{
  ...
  add_TOC_to_workbook(wb,
                      list(beta_table(mtcars,"Test 1"),
                           beta_table(mtcars,"Test 2")),
                      c("test1","test2"))
...
test_that("add_TOC_to_workbook() pulls the titles and writes them if they exist",{
  ...
  writeData(wb,toc_sheet,mtcars)
  add_TOC_to_workbook(wb,
                      list(beta_table(mtcars,"test_1"),
                                     beta_table(mtcars,"test_2"),
                                     mtcars,
                                     beta_table(mtcars,"test_3")),
                      c("1","2","3","4"))
...
test_that("add_TOC_to_workbook() adds the sheet names and titles from an existing workbook",{
  ...
  # now lets copy what we do in beta_to_workbook and append all the old sheets
...
test_that("data_to_worksheet() generates data correctly",{
  test_df <-
    ...
    mutate(mpg = mpg,
           cyl = as.integer(cyl),
           disp = beta_double(disp, dp = 0),
           hp = beta_vector(hp),
           drat = beta_double(drat, dp = 4),
           wt = beta_percent(wt,dp =2),
...
test_that("beta_table_to_sheet() works", {
  test_file <- test_path("test_xlsx_files","beta_table_to_sheet.xlsx")
  # now get it as workbook
  wb_test <- openxlsx::loadWorkbook(test_file)
  # now create a work
  wb <- createWorkbook()
  ...
  table_no_title_footnote <- beta_table(test_df)
  table_no_footnote <- beta_table(test_df,
                                  "Test title")
  table_all <- beta_table(test_df,
                          "Test title",
                          "Test footnote")
  # now add 5 different workbooks
  beta_table_to_sheet(table_no_title_footnote,
                      wb,
                      "Test 1 - wo TOC Footnote Title")
  beta_table_to_sheet(table_no_footnote,
                      wb,
                      "Test 2 - wo TOC Footnote",
                      TOC = TRUE)
  beta_table_to_sheet(table_no_footnote,
  ...
  skip_on_cran()
  test_output <- test_path("_output_excel_snaps/beta_table_to_sheet.xlsx")
  # we use openxlsx to write it
  saveWorkbook(wb,
               test_output,
               overwrite = TRUE)
...
test_that("beta_table_to_sheet() aborts if the sheets don't exist", {
  wb <- createWorkbook()
  append_worksheet("mhmm",wb)

  expect_snapshot(beta_table_to_sheet(beta_table(data.frame()),
                                      wb,
                                      "test"),
                  error = TRUE)
})

test_that("beta_table_to_sheet() creates the approriate error messages when x is missing columsn or rows", {
  wb <- createWorkbook()
  append_worksheet("test",wb)

  expect_snapshot(beta_table_to_sheet(beta_table(data.frame()),wb,"test"),
                  error = TRUE)
  expect_snapshot(beta_table_to_sheet(beta_table(data.frame("data"=character())),
                                      wb,
                                      "test"))
...
test_that("beta_to_workbook() creates an error when you pass an empty list", {

  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(list()),
                  error = TRUE)
})


test_that("beta_to_workbook() checks the sheet_name type correctly", {

  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,sheet_name = 1),
                  error = TRUE)
})

test_that("beta_to_workbook() checks the sheet_name type correctly", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,"test",1),
                  error = TRUE)
})
test_that("beta_to_workbook() checks the overwrite_sheet type correctly", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,"test",overwrite_sheet = "wee"),
                  error = TRUE)
})
test_that("beta_to_workbook() checks the TOC type correctly", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,"test",TOC = "wee"),
                  error = TRUE)
})
test_that("beta_to_workbook() checks the TOC_title type correctly", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,"test",
                                   TOC = TRUE,
                                   TOC_title = FALSE),
                  error = TRUE)
})

test_that("beta_to_workbook() checks the excel_data_table type correctly", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars,"test",
...
test_that("beta_to_workbook() errors when you pass a sheet_name, and a list of data", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(list(mtcars,mtcars),"test"))
})

test_that("beta_to_workbook() errors when you pass the wrong type in a list", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(list("asd",123)),
                  error = TRUE)
})

test_that("beta_to_workbook() errors when you pass the wrong type", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(123),
                  error = TRUE)
})

test_that("beta_to_workbook() errors when you pass a single object but not a sheetname", {
  # test that an empty list generates an error
  expect_snapshot(beta_to_workbook(mtcars),
                  error = TRUE)
})

test_that("beta_to_workbook() creates a file from a list of beta_tables w/o TOC", {
  test_file <- test_path("test_xlsx_files","beta_table_to_workbook_no_TOC.xlsx")
  # now get it as workbook
  wb_test <- openxlsx::loadWorkbook(test_file)
  # now create a work
  test_list <- list(
    "Test 1 - no footnote title" = beta_table(mtcars),
    "Test 2 - no Footnote" = beta_table(mtcars,
                                        "Test title"),
    "Test 3 - all" = beta_table(mtcars,
                                "Test title",
                                "Test footnote")
  )

  wb <- beta_to_workbook(test_list)
  expect_true(equal_workbook_sheets(wb,wb_test))
  # expect_true(equal_workbook_data(wb,wb_test))
  # now see if they have the same data
  # expect_true(equal_openxlsx_workbook(wb,wb_test))
  ...
  skip_on_cran()
  test_output <- test_path("_output_excel_snaps/beta_table_to_workbook.xlsx")
  # we use openxlsx to write it
  saveWorkbook(wb,
               test_output,
               overwrite = TRUE)
...
test_that("beta_to_workbook() creates a file from a list of beta_tables", {
  test_file <- test_path("test_xlsx_files","beta_table_to_workbook_TOC.xlsx")
  # now get it as workbook
  wb_test <- openxlsx::loadWorkbook(test_file)
  # now create a work
  test_list <- list(
  ...
  skip_on_cran()
  test_output <- test_path("_output_excel_snaps/beta_table_to_workbook_TOC.xlsx")
  # we use openxlsx to write it
  saveWorkbook(wb,
               test_output,
               overwrite = TRUE)
...
test_that("beta_to_workbook() appends to an existing workbook", {
  ...
  wb <- beta_to_workbook(test_list,old_wb = wb_old, TOC = TRUE)
  ...
  skip_on_cran()
  test_output <- test_path("_output_excel_snaps/beta_table_to_workbook_old_wb.xlsx")
  # we use openxlsx to write it
  saveWorkbook(wb,
               test_output,
               overwrite = TRUE)
...
test_that("beta_to_workbook() appends to an existing and makes the correct TOC", {

  tmp_file <- tempfile(fileext = ".xlsx")
  on.exit(unlink(tmp_file))
  bt <- beta_table(mtcars,"title 1","footnote")
  ...
  skip_on_cran()
  test_output <- test_path("_output_excel_snaps/beta_table_to_workbook_old_wb_2.xlsx")
  # we use openxlsx to write it
  saveWorkbook(wb,
               test_output,
               overwrite = TRUE)
