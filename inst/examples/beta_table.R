library(BETAexcel)
library(dplyr)
# Create a beta_table, we set the footnotes and the title
# It converts to the beta types by default
x <- beta_table(mtcars,
...
x
# You can use mutate and summarise with beta_tables and they are preserved
x |>
...
x <- x |>
  ...
  update_theme(title_format = beta_format(font_size = 14))
\dontrun{
  write_xlsx(x,
             "example.xlsx",
             "A example sheet",
